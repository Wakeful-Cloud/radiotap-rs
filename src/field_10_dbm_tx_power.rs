...
///
/// See [www.radiotap.org/fields/dBm%20TX%20power.html](https://www.radiotap.org/fields/dBm%20TX%20power.html)
/// for more information.
#[derive(Clone, Debug, Default, DekuRead, DekuWrite, PartialEq)]
#[deku(endian = "little", bit_order = "lsb")]
pub struct DbmTxPowerField {
  // Transmit power expressed as dBm (decibels from a 1 milliwatt reference). This is the absolute
  // power level measured at the antenna port.
  pub power: i8,
}

impl<V> RadiotapFieldTrait<V> for DbmTxPowerField {
  fn get_identifiers(&self) -> RadiotapFieldTraitIdentifiers {
    RadiotapFieldTraitIdentifiers { bit_index: 10 }
  }
  ...
  fn decode(&self, reader: &mut FieldReader) -> Result<RadiotapField<V>, RadiotapError> {
    // Decode the field
    let decoded = DbmTxPowerField::from_reader((reader, 0))?.1;

    Ok(RadiotapField::DbmTxPower(decoded))
  }
...
mod tests {
  ...
  /// Encoding test cases
  const ENCODING_CASES: &[(&[u8], &DbmTxPowerField)] = &[
    // Simple
    (
      &[
        0x34, // Power
      ],
      &DbmTxPowerField { power: 0x34 },
    ),
  ];

  #[rstest]
  #[case::simple(ENCODING_CASES[0].0, ENCODING_CASES[0].1)]
  fn test_decode_field(#[case] encoded: &[u8], #[case] expected: &DbmTxPowerField) {
    // Decode the bytes
    let decoded = DbmTxPowerField::from_bytes((encoded, 0)).unwrap().1;
  ...
  fn test_encode_field(#[case] field: &DbmTxPowerField, #[case] expected: &[u8]) {
    // Encode the field
